...
def extract_summary(pdf_bytes: bytes) -> pd.DataFrame:
    ...
    rx_minus_iva21 = re.compile(
        r"[−-]\s*IVA\s*21,00%[^0-9]*(\d{1,3}(?:\.\d{3})*,\d{2})", re.IGNORECASE
    )
    ...
    with pdfplumber.open(tmp_path) as pdf:
        for page in pdf.pages:
            ...
            for m in rx_perc_rg333.finditer(txt):
                tot_perc += _to_float(m.group(1))
            for m in rx_ret_iibb.finditer(txt):
                tot_ret_ib += _to_float(m.group(1))

    iva21_final = tot_iva_arancel + tot_menos_iva
    base21 = round(iva21_final / 0.21, 2) if iva21_final else 0.0
    base105 = round(tot_iva_costo / 0.105, 2) if tot_iva_costo else 0.0
...
def build_pdf(resumen_df: pd.DataFrame, out_path: str, titulo: str):
    ...
    story.append(
        Paragraph(
            f"Generado: {datetime.datetime.now().strftime('%d/%m/%Y %H:%M')}", normal
        )
    )
    ...
    for _, row in resumen_df.iterrows():
        data.append([row["Concepto"], _fmt_money(float(row["Monto"]))])
